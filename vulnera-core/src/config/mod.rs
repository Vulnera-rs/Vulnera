...
pub struct SastConfig {
    ...
    /// Preferred parser frontend for JavaScript/TypeScript (`tree_sitter` or `oxc_preferred`)
    pub js_ts_frontend: Option<String>,
    /// Minimum severity threshold to report (critical|high|medium|low|info)
    pub min_finding_severity: Option<String>,
    /// Minimum confidence threshold to report (high|medium|low)
    pub min_finding_confidence: Option<String>,
    /// Require data-flow evidence for findings generated by data-flow rules
    pub require_data_flow_evidence_for_dataflow: bool,
    /// Require non-empty recommendations for reported findings
