...
impl FileCacheRepository {
    ...
    async fn check_and_cleanup_entry(&self, path: &PathBuf) -> Result<bool, ApplicationError> {
        let content = fs::read(path).await.map_err(ApplicationError::Io)?;

        // Decompress if needed
        let decompressed_content =
            if self.enable_compression && content.len() > 4 && &content[0..4] == b"CMP\0" {
        ...
        while let Some(entry) = entries.next_entry().await.map_err(ApplicationError::Io)? {
            ...
            match fs::read(&path).await {
                Ok(content) => {
                    ...
                    match serde_json::from_slice::<CacheEntry<serde_json::Value>>(
...
impl CacheService for FileCacheRepository {
        ...
        T: serde::de::DeserializeOwned + Send,
    {
        ...
        let entry: CacheEntry<serde_json::Value> = serde_json::from_slice(&decompressed_content)
            .map_err(|e| {
