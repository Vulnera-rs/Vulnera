...
impl CacheBackend for DragonflyCache {
    async fn get_raw(&self, key: &str) -> Result<Option<Vec<u8>>, String> {
        DragonflyCache::get_raw(self, key)
            .await
            .map_err(|e| e.to_string())
    }

    async fn set_raw(&self, key: &str, value: &[u8], ttl: Duration) -> Result<(), String> {
        DragonflyCache::set_raw(self, key, value, ttl)
            .await
            .map_err(|e| e.to_string())
    }

    async fn delete(&self, key: &str) -> Result<(), String> {
        DragonflyCache::delete(self, key)
            .await
            .map(|_| ())
            .map_err(|e| e.to_string())
    }
    ...
    async fn delete_by_pattern(&self, pattern: &str) -> Result<usize, String> {
        DragonflyCache::delete_by_pattern(self, pattern)
            .await
            .map_err(|e| e.to_string())
    }
}
