...
use vulnera_secrets::infrastructure::verification::{GitHubVerifier, GitLabVerifier, SecretVerifier, VerificationResult};
...
async fn test_github_verifier_success() {
    ...
    let result = verifier.verify("valid_token", &SecretType::GitHubToken, Duration::from_secs(1)).await;
...
async fn test_github_verifier_invalid() {
    ...
    let verifier = GitHubVerifier::with_base_url(mock_server.uri());
    let result = verifier.verify("invalid_token", &SecretType::GitHubToken, Duration::from_secs(1)).await;
...
async fn test_gitlab_verifier_success() {
    ...
    let result = verifier.verify("valid_token", &SecretType::GitLabToken, Duration::from_secs(1)).await;
