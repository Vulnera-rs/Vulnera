...
use chrono::Utc;
use vulnera_core::domain::vulnerability::value_objects::{Ecosystem, Version, Severity, VulnerabilityId, VersionRange, VulnerabilitySource};
use vulnera_core::domain::vulnerability::entities::{Vulnerability, AffectedPackage, Package};
        ...
        if let Some(result) = guard.get(&key) {
            match result {
                ...
                Err(e) => match e {
                    ...
                    RegistryError::Http { message, status } => Err(RegistryError::Http { message: message.clone(), status: *status }),
...
async fn test_recommend_latest_stable() {
    ...
    registry.add_versions(Ecosystem::Npm, "express", vec!["4.17.1", "4.17.2", "4.18.0", "4.18.1"]);
    ...
    assert_eq!(recommendation.most_up_to_date_safe, Some(Version::parse("4.18.1").unwrap()));
    assert_eq!(recommendation.nearest_safe_above_current, Some(Version::parse("4.17.1").unwrap())); // Current is safe
}
...
async fn test_recommend_avoid_vulnerable() {
    ...
    assert_eq!(recommendation.most_up_to_date_safe, Some(Version::parse("4.18.1").unwrap()));
    assert_eq!(recommendation.nearest_safe_above_current, Some(Version::parse("4.18.0").unwrap()));
}
...
async fn test_recommend_prerelease_exclusion() {
    ...
    assert_eq!(recommendation.most_up_to_date_safe, Some(Version::parse("4.17.1").unwrap()));
}

#[tokio::test]
async fn test_recommend_registry_unavailable_fallback() {
    let registry = Arc::new(MockRegistryClient::new());
    registry.set_error(Ecosystem::Npm, "express", RegistryError::Http { message: "Timeout".to_string(), status: Some(504) });
    ...
    assert!(recommendation.notes.iter().any(|n| n.contains("registry unavailable")));
}
...
async fn test_recommend_all_vulnerable() {
    ...
    assert!(recommendation.notes.iter().any(|n| n.contains("no known safe version")));
}
